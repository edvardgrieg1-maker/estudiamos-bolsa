...
exports.handler = async function(event) {
  ...
  try {
    ...
    const subscriberResult = await new Promise((resolve, reject) => {
      const options = {
        ...
        headers: {
          ...
          'Content-Length': Buffer.byteLength(subscriberPayload)
        }
      };
      ...
      req.on('error', reject);
      req.write(subscriberPayload);
      req.end();
    });

    if (subscriberResult.status !== 200 && subscriberResult.status !== 201) {
      return {
        statusCode: 400,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ success: false, error: subscriberResult.body })
      };
    }
    ...
    await new Promise((resolve, reject) => {
      const options = {
        hostname: 'connect.mailerlite.com',
        path: `/api/subscribers/${subscriberId}/groups/1871849983420437831`,
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
